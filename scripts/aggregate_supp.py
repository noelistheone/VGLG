...
METATSF_DIR = REPO / "logs" / "metatsf_tuned2"
...
SUPP_RECIPE = {
    ...
    # plateaus by epoch 1-4), and wd=1e-3 was the sole scout winner (Traffic
    # h96 0.566->0.501, -11.5%; electricity -4.1%; ETT flat; 1e-2 over-regs).
...
def main() -> None:
    ...
    lines.append("Each baseline was rerun under a recipe closer to its original paper's training "
                 ...
                 "MetaTSF variants were re-tuned in two rounds under a single shared recipe: an "
                 "lr scout (round 1) fixed `30 ep / lr 1e-3 / cosine / patience 8`, and a "
                 "regularization scout (round 2) added `weight_decay 1e-3` after the runs were "
                 "found to overfit heavily. All four mixers share this identical recipe (preserving "
                 "the same-backbone thesis). DLinear is the only model unchanged from main. "
    ...
    for m in SUPP_MODELS + METATSF_MIXERS:
        ...
        wd = r.get("wd", 0.0)
        wd_str = f" / wd {wd:.0e}" if wd else ""
        supp_str = (f"{r['epochs']} ep / lr {r['lr']:.0e} / {r['lradj']} / pat {r['patience']}{wd_str}")
    ...
    lines.append("")
    lines.append("### 4. MetaTSF retuned in two rounds — lr, then regularization")
    lines.append("")
    lines.append("The MetaTSF variants were tuned in two rounds, always with one recipe shared "
    ...
    lines.append("Traffic (N=862) was essentially the *entire* Avg(7) deficit between MetaTSF and "
                 ...
                 "**Trade-off, stated honestly:** wd = 1e-3 (scouted on h96 only) is too strong at "
                 "the longest horizon. 25 of 112 MetaTSF cells regress versus the main baseline, 7 "
                 "of them by more than 3 % — almost all at h720 on ETTh2 / Weather / ETTm1 / ETTm2 "
                 "(worst: MLP · ETTh2 · h720, +16 %). These look like under-fitting rather than "
                 "noise: a 720-step horizon needs more effective capacity, not less. The Traffic "
                 "and short-horizon gains outweigh them, so Avg(7) still improves for all four "
                 "mixers, but a horizon-aware weight decay (smaller wd as pred_len grows) would "
                 "likely recover those cells.")
    lines.append("")
