...
def kd_loss_bundle(
    ...
    """Compute (trend, freq, diff) KD losses sharing one rfft per tensor.

    The trainer hot loop previously called the three top-level fns, which
    redundantly took rfft of `student_pred`/`teacher_pred` twice each. This
    fuses them; for pred_len=720 it removes 2× O(N log N) per batch.
    """
    s_fft = torch.fft.rfft(student_pred, dim=1)
    t_fft = torch.fft.rfft(teacher_pred, dim=1)
    ...
    l_freq = F.mse_loss(s_fft.abs(), t_fft.abs())

    s_diff = student_pred[:, 1:] - student_pred[:, :-1]
    t_diff = teacher_pred[:, 1:] - teacher_pred[:, :-1]
    l_diff = F.mse_loss(s_diff, t_diff)

    return l_trend, l_freq, l_diff
...
def total_distill_loss(
    ...
    l_trend, l_freq, l_diff = kd_loss_bundle(pred, teacher_pred)
